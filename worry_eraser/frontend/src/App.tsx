...
const AppContent: React.FC = () => {
  const [currentAgent, setCurrentAgent] = useState<'zhouyu' | 'luxun'>('zhouyu');
  ...
  const handleAgentChange = useCallback((agent: 'zhouyu' | 'luxun') => {
  ...
  return (
    <div className="container">
      {demoMode && (
        <div className="demo-mode">演示模式</div>
      )}
      ...
      {showTrilogy && (
        <Suspense fallback={<LoadingFallback />}>
          <TrilogyController
            ...
            reportData={report}
            memoryContent={messages.length > 1 ? messages[messages.length - 2].content : undefined}
            onComplete={handleTrilogyComplete}
          />
        </Suspense>
      )}
